...
- **`com.gabon.jooq`** 生成代码维持现状,是 schema 的类型化投影,不是模块。
...
- **Access JWT**(jjwt 0.12,HS256):claims = `sub` / `typ`(customer|admin)/ `roles` / `jti`(uuidv7)/ `iat` / `exp`(**15 分钟**)。签名密钥从文件注入(`/run/secrets/*`,SOPS 流程),不进 git;测试用 test 固定键;prod 缺失即启动失败(fail fast)。
- **Refresh token**:256-bit 随机不透明串,**旋转式**——每次 refresh 换新、旧 token 标记 `rotated_at`;**重用检测**:已 rotated 的 token 再次出现 → 吊销整个 family(防重放)。TTL 30 天。
- **登出**:吊销 refresh family + 当前 access `jti` 进 Valkey 黑名单(TTL = 剩余有效期)。
...
- **算法定案**:RFC 6238,JDK `Mac` 实现,**不自创算法**;参数写死:**30s 步长 / 6 位 / HMAC-SHA1 / 验证窗口最多 [-1, 0, +1]**;RFC 6238 Appendix B 官方测试向量做契约测试。
...
- **防重放**:成功验证后写 `totp_last_used_step`,同一 time step 内拒绝再次接受(RFC 6238 one-time validation)。
...
| TOTP 单测 | RFC 6238 Appendix B 向量;窗口边界;同 step 重放拒绝;加解密往返 + AAD 篡改拒绝(注入 `java.time.Clock` 保证确定性) |
| 身份域集成测试(PG+Valkey) | register→login→me 闭环;5 次失败→锁定(锁定期内正确密码也 401);refresh 旋转 + 重用检测吊销全 family;logout→jti 黑名单生效;改密→全 session 吊销;admin TOTP enroll→confirm→登录;Valkey 不可用→503 fail-closed |
| 回归 | 既有 9 测试随包迁移后全绿 |
